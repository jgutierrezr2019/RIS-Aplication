...
% Datos
U = 2*5; % Numero de usuarios de la simulación
...
%% Simulación de N usuarios en servicio multicast con caso ideal de Procesamiento de señal
% Calcular SNR de los usuarios a los que la RIS transmite
for i = 1:U/2
    % Pérdidas de propagación
    alpha = 10*log10(Aeff/(4*pi*d_txtoRIS^2)); % Pathloss a RIS
    beta = 10*log10(Aeff/(4*pi*d_RIStoUser_rdn(i)^2)); % Pathloss desde RIS
...
fprintf('El escenario de haz reflejado ensanchado tiene SNR de %d dB de mínimo\n', min(SNRb(1,1:U/2)));
fprintf('El escenario de haz pincel reflejado tiene SNR de %d dB de mínimo\n', min(SNRb(2, 1:U/2)));
fprintf('El escenario de Bjorson con ganancia de RIS ideal tiene SNR de %d dB de mínimo\n', min(SNRb(3, 1:U/2)));
